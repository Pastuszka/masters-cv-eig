...
for(degree in exp_degrees){
  for(i in 1:n_samples){
    ...
    write.table(A, paste0('samples_poiss/graph_point_', degree, '_', i, '.table'))
    
    sbm_exp <- dcsbm(theta = theta_exp, B = B, expected_degree = degree)
    A <- as.matrix(sample_sparse(sbm_exp, poisson_edges = T, allow_self_loops = F))
    write.table(A, paste0('samples_poiss/graph_exp_', degree, '_', i, '.table'))
    
    sbm_pareto <- dcsbm(theta = theta_pareto, B = B, expected_degree = degree)
    A <- as.matrix(sample_sparse(sbm_pareto, poisson_edges = T, allow_self_loops = F))
    write.table(A, paste0('samples_poiss/graph_pareto_', degree, '_', i, '.table'))
